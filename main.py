...
def parse_args():
    ...
    parser.add_argument('--class_based_adapter', action='store_true',
                       help='Whether to use adapter-based training')
...
def main(args=None):
    ...
    if isinstance(dataset, ContinualDataset):
        if args.use_adapter:
            if args.class_based_adapter:
                from utils.training import train_class_adapter  # Import the adapter training function
                train_class_adapter(model, dataset, args)
            else:
